...
<Card title="Chat Message (Streaming)" icon="waveform" href="#chatbot-message-stream">
  **POST** `/chatbot/messages/stream`<br/>
  Real-time SSE streaming for progressive responses
</Card>
...
### Streaming Chat {#chatbot-message-stream}

Test real-time SSE streaming from the chatbot:
...
curl -N \
  ...
  -d '{
    "message": { "message": "Write a short LinkedIn post on remote work" },
    "mode": "prompt2post",
...
async function main() {
  const res = await fetch('https://postreachai-ekc4e7fke7bacehe.southeastasia-01.azurewebsites.net/api/v1/chatbot/messages/stream', {
    method: 'POST',
    headers: {
...
<Check>
For full streaming details and event breakdown, see the dedicated page: `/api-reference/chatbot/chat-message-stream`.
</Check>
